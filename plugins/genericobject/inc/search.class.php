<?php
/*
 * @version $Id: search.class.php 18301 2012-04-18 11:22:12Z moyo $
 -------------------------------------------------------------------------
...
*/
   ...
   **/
   static function addDefaultToView ($itemtype) {
      ...
      if( stripos($itemtype, 'PluginGenericobject') !== false ) {
      	$items = &self::getOptions($itemtype);
      	foreach($items as $key => $val) {
      		if( ($val['name'] == 'Model') || ($val['name'] == 'Type')
      		 || ($val['name'] == 'User') || ($val['name'] == 'Manufacturer')
      		 || ($val['name'] == 'Serial Number') ) {
      			array_push($toview, $key);
      		}
      	}
      }
      return $toview;
