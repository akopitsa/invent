...
function plugin_domains_AssignToTicket($types) {
	...
	if (plugin_domains_haveRight("open_ticket","1"))
		$types['PluginDomainsDomain']=$LANG['plugin_domains']['title'][1];
	return $types;
}
...
function plugin_domains_getDropdown() {
	...
	if ($plugin->isActivated("domains"))
		return array('PluginDomainsDomainType'=>$LANG['plugin_domains']['setup'][2]);
	else
		return array();
}
...
function plugin_domains_getAddSearchOptions($itemtype) {
   ...
   if (in_array($itemtype, PluginDomainsDomain::getTypes(true))) {
      if (plugin_domains_haveRight("domains","r")) {
         ...
         $sopt[4410]['name']=$LANG['plugin_domains']['title'][1]." - ".$LANG['plugin_domains'][7];
         ...
         $sopt[4411]['name']=$LANG['plugin_domains']['title'][1]." - ".$LANG['plugin_domains'][12];
...
function plugin_domains_MassiveActions($type) {
	...
	switch ($type) {
		case 'PluginDomainsDomain':
			return array(
				// Specific one
				"plugin_domains_install"=>$LANG['plugin_domains']['setup'][9],
				"plugin_domains_desinstall"=>$LANG['plugin_domains']['setup'][10],
				"plugin_domains_transfert"=>$LANG['buttons'][48],
				);
         break;
	}
	if (in_array($type, PluginDomainsDomain::getTypes(true))) {
			return array("plugin_domains_add_item"=>$LANG['plugin_domains']['setup'][18]);
		}
	return array();
}
	...
	switch ($options['itemtype']) {
		case 'PluginDomainsDomain':
			switch ($options['action']) {
				...
				case "plugin_domains_install":
					Dropdown::showAllItems("item_item",0,0,-1,PluginDomainsDomain::getTypes(true));
					echo "<input type=\"submit\" name=\"massiveaction\" class=\"submit\" value=\"".$LANG['buttons'][2]."\" >";
               break;
				case "plugin_domains_desinstall":
					Dropdown::showAllItems("item_item",0,0,-1,PluginDomainsDomain::getTypes(true));
               echo "<input type=\"submit\" name=\"massiveaction\" class=\"submit\" value=\"".$LANG['buttons'][2]."\" >";
               break;
				case "plugin_domains_transfert":
   ...
   $domain_item = new PluginDomainsDomain_Item();
	...
	switch ($data['action']) {
      ...
      case "plugin_domains_install":
         foreach ($data["item"] as $key => $val) {
            if ($val == 1) {
               $input = array('plugin_domains_domains_id' => $key,
      ...
      case "plugin_domains_desinstall":
         foreach ($data["item"] as $key => $val) {
           if ($val == 1) {
               $domain_item->deleteItemByDomainsAndItem($key,$data['item_item'],$data['itemtype']);
            }
         }
			break;
      case "plugin_domains_transfert":
         if ($data['itemtype'] == 'PluginDomainsDomain') {
				foreach ($data["item"] as $key => $val) {
					if ($val == 1) {
                  $domain->getFromDB($key);
                  $type = PluginDomainsDomainType::transfer($domain->fields["plugin_domains_domaintypes_id"],$data['entities_id']);
                  if ($type > 0) {
                     $values["id"] = $key;
