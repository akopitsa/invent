...
if (strpos($_SERVER['PHP_SELF'],"dropdownDatabases.php")) {
	define('GLPI_ROOT', '../../..');
	$AJAX_INCLUDE=1;
...
$where=" WHERE  (`glpi_plugin_databases_databases`.`plugin_databases_databasetypes_id` = '".$_POST['plugin_databases_databasetypes_id']."') ";

if (isset($_POST["entity_restrict"])&&$_POST["entity_restrict"]>=0) {
	$where.=getEntitiesRestrictRequest("AND","glpi_plugin_databases_databases",'',$_POST["entity_restrict"],true);
} else {
	$where.=getEntitiesRestrictRequest("AND","glpi_plugin_databases_databases",'','',true);
}
...
if ($_POST['searchText']!=$CFG_GLPI["ajax_wildcard"])
	$where.=" AND `glpi_plugin_databases_databases`.`name` ".Search::makeTextSearch($_POST['searchText']);
