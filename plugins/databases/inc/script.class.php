...
class PluginDatabasesScript extends CommonDBChild {
   ...
   public $items_id = 'plugin_databases_databases_id';
	...
	static function getTypeName() {
      ...
      return $LANG['plugin_databases'][8];
   }
   ...
   static function displayTabContentForItem(CommonGLPI $item, $tabnum=1, $withtemplate=0) {
      ...
      if ($item->getType()=='PluginDatabasesDatabase') {
         ...
         $self->showScripts($item->getField('id'));
   ...
   function prepareInputForAdd($input) {
	...
	function showForm ($ID, $options=array()) {
      ...
      echo "<td>".$LANG['plugin_databases']['script'][0].": </td>";
      echo "<td>";
      Html::autocompletionTextField($this,"name");	
      echo "</td>";

      echo "<td>".$LANG['plugin_databases']['script'][1].": </td>";
      echo "<td>";
      ...
      echo "<td>".$LANG['plugin_databases']['script'][2].": </td>";
      echo "<td>";
	...
	function showScripts($ID) {
      ...
      $query = "SELECT `".$this->getTable()."`.`name` AS name, 
                        `".$this->getTable()."`.`id`,
                        `".$this->getTable()."`.`plugin_databases_databases_id`, 
                        `".$this->getTable()."`.`path`, 
                        `".$this->getTable()."`.`comment`,`glpi_plugin_databases_scripttypes`.`name` AS type 
               FROM `".$this->getTable()."` ";
      $query.= " LEFT JOIN `glpi_plugin_databases_scripttypes` ON (`glpi_plugin_databases_scripttypes`.`id` = `".$this->getTable()."`.`plugin_databases_scripttypes_id`)";
      ...
      $row_num=1;
      if ($number !="0") {
         echo "<form method='post' name='form_scripts$rand' id='form_scripts$rand' action=\"./script.form.php\">";
         echo "<div align='center'><table class='tab_cadre_fixe'>";
         echo "<tr><th colspan='5'>".$LANG['plugin_databases'][8].":</th></tr>";
         $sel="";
         ...
         echo "<th>&nbsp;</th>";
         echo "<th>".$LANG['plugin_databases']['script'][0]."</th>";
         echo "<th>".$LANG['plugin_databases']['script'][1]."</th>";
         echo "<th>".$LANG['plugin_databases']['script'][2]."</th>";
         ...
         if ($number && $canedit) {
            if ($this->canCreate()) {
               Html::openArrowMassives("form_scripts$rand", true);
               Html::closeArrowMassives(array('delete_script' => $LANG['buttons'][6]));
